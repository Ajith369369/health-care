...
export default (
  ...
  const getAllAppointments = async (
    ...
    try {
      ...
      return res.status(HttpStatus.OK).json({ success: true, appointments });
    } catch (error) {
      next(error);
    }
  };
  ...
  const addDepartment = async (
    ...
    try {
      ...
      const department = await addOneDepartment(
        departmentName,
        isListed,
  ...
  const updateDepartment = async (
    ...
    try {
      ...
      const update = await departmentUpdate(
        departmentName,
        id,
        dbDepartmentRepository
      );
      return res
        .status(HttpStatus.OK)
        .json({ success: true, update, message: "update Successfully" });
    } catch (error) {
      next(error);
    }
  };
  ...
  const unlistDepartment = async (
    ...
    try {
      const { id } = req.params;
      const updatedDepartment = await departmentUnlist(
        id,
        dbDepartmentRepository
      );
      return res.status(HttpStatus.OK).json({
