...
import {
  ...
  getPatients,
} from "../app/use-cases/doctor/doctorRead";
import {
  DoctorRejected,
  getDoctorProfile,
  updateDoctor,
...
import {
  ...
  fetchPrescriptionForDoctor,
} from "../app/use-cases/Prescription/prescriptionUseCase";
...
const doctorController = (
  ...
  const dbPrescriptionRepository = prescriptionDbRepository(
  ...
  const googleSignIn = async (
    ...
    try {
      ...
      res
        ...
        .json({ message: "login success", user, accessToken: accessToken });
    } catch (error) {
      next(error);
    }
  };
  ...
  const addSlot = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const response = await addTimeSlot(data, dbTimeSlotRepository);
  ...
  const fetchPrescription = async (
    ...
    try {
      const { id } = req.params;
      const data = id;
      const response = await fetchPrescriptionForDoctor(
        data,
        dbPrescriptionRepository
      );
  ...
  const deletePrescription = async (
    ...
    try {
      ...
      const response = await deletePrescriptionData(
        prescriptionId,
        dbPrescriptionRepository
      );
      res.status(HttpStatus.OK).json({ sucess: true, response });
