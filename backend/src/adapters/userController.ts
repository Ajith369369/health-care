...
const userController = (
  ...
  const doctorPage = async (
    ...
    try {
      const { searchQuery, department, selectedDate, selectedTimeSlot } =
        req.query;
  ...
  const fetchPrescription = async (
    ...
    try {
      ...
      const response = await fetchPrescriptionUsecase(
        data,
  ...
  const fetchDocuments = async (
    ...
    try {
      const { id } = req.params;
      const documents = await getDocuments(id, dbPrescriptionRepository);
