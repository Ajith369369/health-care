...
export const doctorRepositoryMongodb = () => {
  ...
  const getFilteredDoctors = async ({
    ...
    if (selectedDate) {
      ...
      const dateFilteredTimeSlots = await timeSlots
        .find({
          startDate: { $lte: date },
          endDate: { $gte: date },
          available: true,
        ...
        .select("doctorId");
      doctorIds = dateFilteredTimeSlots.map((slot: any) =>
    ...
    if (selectedTimeSlot) {
      ...
      const timeFilteredTimeSlots = await timeSlots
        .find({
          available: true,
      ...
      const timeFilteredDoctorIds = timeFilteredTimeSlots.map((slot: any) =>
        slot.doctorId.toString()
      );
      ...
      if (selectedDate) {
        ...
        const dateFilteredDoctorIds = dateFilteredTimeSlots.map((slot: any) =>
          slot.doctorId.toString()
        );
        doctorIds = doctorIds.filter(
          (id) =>
            timeFilteredDoctorIds.includes(id) &&
            dateFilteredDoctorIds.includes(id)
        );
      } else {
  ...
  ) => await Doctor.findByIdAndUpdate(id, updateData, { new: true });

  const getAllDoctors = async () => await Doctor.find({ isVerified: true });

  const getAllAppointments = async () =>
    await Booking.find({ appointmentStatus: "Booked" });
