...
const LineChart: React.FC<LineGraphProps> = ({
  doctors,
  users,
  appointments,
}) => {
  ...
  useEffect(() => {
    ...
    if (view === "monthly") {
      ...
      const appointmentsMonthlyCounts = generateMonthlyCounts(
        appointments,
        "createdAt"
      );
      ...
      const appointmentsYearlyCounts = generateYearlyCounts(
        appointments,
        "createdAt"
      );
      ...
      const completeYearlyCounts = (counts: Record<string, number>) => {
        const completeCounts = yearlyLabels.map(
          (year: any) => counts[year] || 0
        );
  ...
  const maxCount =
    Math.max(...appointmentsData, ...usersData, ...doctorsData) + 3;
      ...
      <div className="flex mb-4">
        <button
          className={`mr-2 p-2 rounded ${
            view === "monthly" ? "bg-blue-900 text-white" : "bg-gray-200"
          }`}
        ...
        <button
          className={`p-2 rounded ${
            view === "yearly" ? "bg-blue-900 text-white" : "bg-gray-200"
          }`}
